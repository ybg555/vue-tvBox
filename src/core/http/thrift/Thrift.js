 ...
 */
Thrift.TApplicationException = function(message, code,msgCode,busCode,errorMessage) {
  this.message = message;
  this.code = typeof code === "number" ? code : 0;
  this.msgCode = typeof code === "number" ? code : 0;
  this.busCode = typeof code === "number" ? code : 0;
...
Thrift.TApplicationException.prototype.write = function(output) {
  ...
  if (this.msgCode) {
    output.writeFieldBegin('msgCode', Thrift.Type.I32, 3);
    output.writeI32(this.msgCode);
    output.writeFieldEnd();
  }

  if (this.busCode) {
    output.writeFieldBegin('busCode', Thrift.Type.I32, 4);
    output.writeI32(this.busCode);
    output.writeFieldEnd();
  }
...
Thrift.TApplicationException.prototype.getMsgCode = function() {
  return this.msgCode;
};

Thrift.TApplicationException.prototype.getBusCode = function() {
  return this.busCode;
};

Thrift.TApplicationException.prototype.getErrorMessage = function() {
...
Thrift.TXHRTransport.prototype = {
  ...
  flush: function(async, callback) {
        ...
        (function() {
          ...
          return function() {
            ...
            if (this.readyState == 4 && this.status == 200) {
              ...
              var linkRes = /^\[\d+,"([^"]+)/.exec(sendBuf);
              self.setRecvBuffer(
                '[1,"'+linkRes[1]+'",3,0,{"1":{"str":"500:::0:::网络异常，请稍后再试"},"5":{"str":"网络异常，请稍后再试"},"3":{"i32":500},"4":{"i32":0},"2":{"i32":6}}]'
              );
              clientCallback();
            }
          };
    ...
    xreq.setRequestHeader("x-client-id", this.options['x-client-id']);
    xreq.setRequestHeader("x-client-type", this.options['x-client-type']);
    xreq.setRequestHeader("x-client-os", "web");

    xreq.setRequestHeader("x-client-os-version", "0");
    xreq.setRequestHeader("x-client-channel", "0");
    xreq.setRequestHeader("x-client-hardware", "0");
    xreq.setRequestHeader("x-client-version-name", "0");
    xreq.setRequestHeader("x-client-version-code", "0");

    xreq.setRequestHeader("x-request-time", currTime);

    var t_token = cookie.getCookie('t-token');
    if(t_token){
      xreq.setRequestHeader("x-manager-token", t_token);
    }else{
      t_token = "null";
      xreq.setRequestHeader("x-manager-token", 'null');
    }

    var signature = md5(t_token + "ever$!@grande*&" + currTime).toUpperCase();
    xreq.setRequestHeader("x-security-timestamp", currTime);
    xreq.setRequestHeader("x-security-signature", signature);
